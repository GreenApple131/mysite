...
def post_list(request, tag_slug=None):
    ...
    if tag_slug:
        tag = get_object_or_404(Tag, slug=tag_slug) #  Внутри обработчика формируем начальный QuerySet, находим все опубликованные статьи и, если указан слаг тега, получаем соответствующий объект модели Tag с помощью метода get_object_or_404()
        object_list = object_list.filter(tags__in=[tag]) # фильтруем и оставляем только статьи с полученным тегом
